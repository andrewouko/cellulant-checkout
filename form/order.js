  ...
  fetch("http://localhost:4000/checkout", {
    ...
    .then((res) => {
      ...
      if (document.getElementById("modal").checked) {
        // checkout_url = encodeURI(
        //   `https://developer.tingg.africa/checkout/v2/modal/?accessKey=${res.accessKey}&params=${res.encrypted_payload}&countryCode=${res.countryCode}`
        // );
        checkout_url = encodeURI(
            `http://localhost:3012?accessKey=${res.accessKey}&params=${res.encrypted_payload}&countryCode=${res.countryCode}`
          );
        var left = 400;
        var top = 200;
        window.open(
        ...
        // checkout_url = encodeURI(
        //   `http://localhost:3010/checkout?accessKey=${res.accessKey}&params=${res.encrypted_payload}&countryCode=${res.countryCode}`
        // );
